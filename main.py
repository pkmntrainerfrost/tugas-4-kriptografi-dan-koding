...
class Ui_MainWindow(object):
    ...
    def loadKey(self):
        ...
        filename = QtWidgets.QFileDialog.getOpenFileName(None,"Pilih file",filter="pri(*.pri)")[0]
    ...
    def encryptPDF(self):
        row = self.currentrow
        save_as = "results/transkrip_"+row["NIM"]+".pdf"
        
        with open('transkrip.html', 'r') as file:
            html_content = file.read()
        for key in self.database_transkrip.columns:
            html_content = html_content.replace(f'[{key}]', row[key])
